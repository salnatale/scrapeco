import requests
from models import LinkedInProfile, LinkedInCompany,Experience,TimePeriod  # Pydantic models
import os
...
def create_transition_event(
    ...
    transition_event = {
        ...
        "location_change": old_experience.location.name != new_experience.location.name if (old_experience.location and new_experience.location) else False,
        "tenure_days": (transition_date - get_start_date(old_experience.time_period)).days,  # Compute tenure
    }
...
def get_start_date(time_period:TimePeriod):
    ...
    return datetime(start_date["year"], start_date["month"], 1) # Create from start data
...
def send_transition_update(transition_event: dict) -> bool:
    ...
    """
    with open("data_schema/druid_transmission_schema.json", "r") as f:
        ingestion_spec = json.load(f)

    ingestion_spec["spec"]["ioConfig"]["inputSource"]["data"] = json.dumps(transition_event)
